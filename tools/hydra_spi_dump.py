...
def hydrabus_setup():
    ...
    # Configure SPI port (default polarity and clock phase, SPI0 device)
    hydrabus.write(b'\x80')
    if b'\x01' not in hydrabus.read(1):
        error("Cannot set SPI device settings, try again or reset hydrabus.")

    # Set spi speed https://github.com/hydrabus/hydrafw/wiki/HydraFW-Binary-SPI-mode-guide#set-spi-speed-0b01100xxx
    hydrabus.write(bytes([0b01100111]))
    if b'\x01' not in hydrabus.read(1):
...
def flash_read(start_addr, sector):
    ...
    return hydrabus.read(sector_size)


def flash_multiple_read(start_addr, sector):
    bufs = [flash_read(start_addr, sector) for _ in range(3)]

    result = b''
    for a, b, c in zip(*bufs):
        result += bytes([take_the_best_of(a, b, c)])  # this is going to fail

    return result


def take_the_best_of(a, b, c):
    # breakpoint()
    if a == b or a == c:
        return a
    elif b == c:
...
def dump_chip():
    ...
    with tqdm.tqdm(desc='Sectors dumped', total=sectors) as progress:
        with open(dump_file, 'wb+') as f:
            while sector < sectors:
                buf = flash_read(start_addr, sector)
                sector += 1
