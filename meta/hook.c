...
int execve(const char *path, char* argv[], char* envp[]) {
	...
	static int (*func_execve) (const char *, char**, char**) = NULL;
	int retval = 0;

	if (! func_execve)
		func_execve = (int (*) (const char*)) dlsym (REAL_LIBC, "execve");

	//retval = func_execve (path, argv, envp);
    retval = 1137;

	DPRINTF ("HOOK: execve(\"%s\"", path);
